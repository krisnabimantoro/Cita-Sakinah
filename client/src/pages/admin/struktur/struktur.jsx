...
const StrukturPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchStructureImage = async () => {
      try {
        ...
        setPreviewImage(`https://api.paudterpaducisa.sch.id/api/storage/uploads/${imageName}`);
      } catch (error) {
  ...
  const closeModal = () => {
    ...
    setPreviewImage(`https://api.paudterpaducisa.sch.id/api/storage/uploads/${strukturImg}`);
    ...
    <>
      <div className="flex flex-col gap-7 w-full">
        <div className="flex items-center justify-between">
          ...
          <Button name="Edit" icon={<FaRegEdit />} color="bg-main" width="w-[127px]" rounded="rounded-full" onClick={openModal} />
        </div>
        <div className="border border-main rounded-lg h-full flex justify-center items-center p-10">
          <img
            src={strukturImg ? `https://api.paudterpaducisa.sch.id/api/storage/uploads/${strukturImg}` : "/path/to/default-image.jpg"}
            alt="img-struktur"
            draggable="false"
          />
        </div>

        <Modal isOpen={isModalOpen} onConfirm={handleSave} onCancel={closeModal} confirm="Simpan">
          <ImageUploadForm
